%% Steady-State Measurement Simulation
...
ini_rate = 0.21; % initiation rate input
elong_rate = 29.0; % elongation rate input
T_term = 65; % termination rate input
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n = 1 : 100 % number of samples
    ...
    fit_res_temp = Steay_State_Model_fitting([ini_val(1),ini_val(2)],ACF_mt,N_stemloop,M_poststemloop); %Larson model fitting

    
    fit_res = [fit_res;[fit_res_temp(:,1),mean((L-X1)./(fit_res_temp(:,2).*stemloopsize))]];
